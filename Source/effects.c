...
//#define LOW_PRECISION_INTERPOLATION
...
IWRAM_CODE void move_table(uint16_t *target, uint8_t *texture, uint16_t *table, int16_t xoffset, int16_t yoffset, uint16_t t)
{
...
IWRAM_CODE void uv_table(uint16_t *target, uint8_t *texture, int *table, uint16_t t)
{
   ...
   for (j = 0; j < (19>>1); j++)
    {
        ...
        {   
            ...
            uvs -= UV_TABLE_ROW;
            uvs -= 3;
#ifdef LOW_PRECISION_INTERPOLATION
            du1 = (((u2 - u1)) >> 3);
            du2 = (((u4 - u3)) >> 3);
            dv1 = (((v2 - v1)) >> 3);
            dv2 = (((v4 - v3)) >> 3);
            ...
            zdu2 = (((d4 - d3) << 16) >> 4);
#endif
            ymul = (((j << 4)) * 120) + (i << 4) - 4 + 120 * 4; /* center screen */
            for (y = 0; y < 16; y += 2)
            {
                tempu1 = tempu2 = tempv1 = tempv2 = tempz1 = tempz2 = 0;
                {      
#ifdef LOW_PRECISION_INTERPOLATION
                    #define INTERPOLATE(add) \
                    { \
